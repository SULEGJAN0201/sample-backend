        ...
        try
            {
                using (var connection = new SqlConnection(_connectionString))
                {
                    ...
                    var allUsers = await connection.QueryAsync<User>("[getallusers]", commandType: CommandType.StoredProcedure);

                    return new BaseResponseService().GetSuccessResponse(allUsers);
                 }
}
